...
export async function findStock(query: string): Promise<FoundStock[]> {
  ...
  const products = await prisma.product.findMany({
    ...
    select: { id: true, code: true, name: true, description: true, unit: true, category: { select: { name: true } } },
    orderBy: { name: "asc" },
    take: 20,
  });
  ...
  const entries = await prisma.stockEntry.findMany({
    where: { productId: { in: products.map((p) => p.id) }, status: "APPROVED", departmentId: null },
    include: {
      ...availabilityInclude,
  ...
  for (const entry of entries) {
    ...
    if (!site) {
      site = { locationId, locationName: entry.location?.name ?? "No site recorded", total: 0, racks: [] };
      sites.push(site);
    }
